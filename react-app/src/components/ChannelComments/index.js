...
import {
	getChannelCommentsThunk,
	clearComments
} from "../../store/channelComments";
...
export default function ChannelComments() {
	...
	const currChannel = useSelector(state => state.channels[channelId]);
	...
	useEffect(() => {
		if (currChannel) {
			//setComments(currChannel.comments);
			setIsLoaded(true);
		}
	}, [currChannel]);
	...
	const currComments = useSelector(state => state.channelComments);
